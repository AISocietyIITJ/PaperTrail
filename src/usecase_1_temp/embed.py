...
def embed_corpus(
    ...
    if device == "cuda":
        ...
        logger.info(f"CUDA Active: utilizing GPU '{gpu_name}' ({vram_gb} GB VRAM) for high-speed batched embedding.")
    else:
        logger.warning("CUDA device not active; proceeding with CPU inference.")
    ...
    if adapter_name and hasattr(model, "load_adapter"):
        try:
            logger.info(f"Attaching adapter: {adapter_name}...")
            model.load_adapter(adapter_name)
        except Exception as e:
            logger.warning(f"Could not load adapter '{adapter_name}' ({e}).")
            logger.warning("The allenai/specter2 adapter is built on an outdated framework and cannot be loaded by modern PEFT. Proceeding with base model embeddings instead.")
    
    texts = (df["title"].fillna("") + ". " + df["abstract"].fillna("")).tolist()
    logger.info(f"Encoding {len(texts)} papers with batch size {batch_size} on {device.upper()}...")
    ...
    for i in range(0, len(vectors_to_upsert), PINECONE_BATCH_SIZE):
        ...
        if i % (PINECONE_BATCH_SIZE * 5) == 0 and i > 0:
            logger.info(f"Upserted {i} vectors...")
 
    logger.info(f"Successfully upserted {len(vectors_to_upsert)} vectors to Pinecone index '{index_name}'.")
...
def generate_embeddings(config_path="config.yaml") -> None:
    ...
    logger.info(f"Loading dataset from {interim_path}...")
