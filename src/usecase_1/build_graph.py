...
def assemble_graph(config_path="config.yaml") -> nx.DiGraph:
    ...
    print(f"Loading directed edges from {directed_path} and dataset from {interim_path}...")
    edges = pd.read_parquet(directed_path)
    df = pd.read_parquet(interim_path)
    
    print(f"Executing vectorized total-order DAG enforcement over {len(edges)} edges...")
    clean_edges, pruned_count = enforce_acyclic_order(edges, df)
    if pruned_count > 0:
        print(f"Pruned {pruned_count} cycle-inducing ambiguous edges via vectorized ordering.")
        
    print("Assembling NetworkX DiGraph...")
    ...
    print(f"Saved validated DAG to {out_path}.")
    return G_final
