...
def get_candidate_edges_pinecone(
    ...
    def fetch_neighbors(node_id):
        for attempt in range(3):
            try:
                ...
                logger.debug(f"Pinecone query failed for node {node_id} (attempt {attempt + 1}/3): {e}")
                time.sleep(1)
        logger.warning(f"Giving up on node {node_id} after 3 failed Pinecone query attempts.")
        return node_id, []
 
    logger.info(f"Querying Pinecone for {num_nodes} nodes using {max_workers} threads...")
    ...
    logger.debug(f"{len(edges)} edges remain after self-loop/bounds/threshold filtering.")
 
    if edges.empty:
        logger.warning("No candidate edges survived filtering; returning empty DataFrame.")
    ...
    logger.debug(f"Deduplicated {before - len(edges)} symmetric pairs, {len(edges)} unique edges remain.")
...
def generate_candidate_edges(config_path="config.yaml") -> pd.DataFrame:
    ...
    logger.info(f"Loading dataset to determine node count from {interim_path}...")
    df = pd.read_parquet(interim_path)
    num_nodes = len(df)
    
    logger.info(f"Searching for candidate edges (k={top_k}, threshold={threshold}) via Pinecone...")
    ...
    logger.info(f"Found {len(edges)} unique candidate edge pairs.")
    
    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    edges.to_parquet(out_path, index=False)
    logger.info(f"Saved candidate edges to {out_path}.")
    return edges
