...
def assign_direction(
    ...
    if "generality_score" not in df.columns:
        print("Computing vocabulary generality scores...")
...
def assign_edge_directions(config_path="config.yaml") -> pd.DataFrame:
    ...
    print(f"Loading candidate edges ({edges_path}) and papers ({interim_path})...")
    edges = pd.read_parquet(edges_path)
    df = pd.read_parquet(interim_path)
    
    print(f"Computing TF-IDF generality scores (max_features={max_feat})...")
    df["generality_score"] = compute_generality_scores(df, max_features=max_feat)
    
    print(f"Assigning edge directions with vectorized optimization (temporal_gap={gap}d, epsilon={epsilon})...")
    ...
    print(f"Directed edges produced: {len(directed)}. Reason breakdown: {counts}")
    
    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    directed.to_parquet(out_path, index=False)
    print(f"Saved directed edges to {out_path}.")
    return directed
